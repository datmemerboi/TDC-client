...
export default function PatientEdit() {
  ...
  const toggleModal = () => { setModal({ show: !modal.show }); };
  return (
    <Fragment>
      <NavBar />
      ...
      <Modal show={modal.show} message={modal.message} clickHandler={toggleModal} />
    </Fragment>
  );
}
...
class PatientEdit extends Component {
      ...
      fetch(url, options)
        ...
        .then((doc) => {
          this.setState({
            ...
            med_history:
              doc.med_history?.length > 0 ? doc.med_history.join(', ') : this.state.med_history,
            current_meds:
              doc.current_meds?.length > 0 ? doc.current_meds.join(', ') : this.state.current_meds
          });
        })
  ...
  handleSubmit = (event) => {
    var patientObj = {
      ...
      med_history: this.state.med_history.length
        ? this.state.med_history.split(',').map((a) => a.trim())
        : null,
      current_meds: this.state.current_meds.length
        ? this.state.current_meds.split(',').map((a) => a.trim())
        : null
    };
    var url =
      process.env?.REACT_APP_ENVIRONMENT === 'local'
          ...
          <div className="hold-together">
            ...
            <div className="right-column">
              ...
              <div>
                ...
                <textarea
                  className="input-area"
                  onChange={this.medHisInput}
                  value={this.state.med_history ?? ''}
                />
              </div>
              <div>
                <label>Current Medicines</label>&nbsp;
                ...
                <textarea
                  className="input-area"
                  onChange={this.currentMedsInput}
                  value={this.state.current_meds ?? ''}
                />
              </div>
            </div>
          </div>
